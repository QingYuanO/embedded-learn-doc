...
**功能说明：**
将WWDG外设的所有寄存器重置为默认值，包括预分频器、窗口值、计数器等配置。
...
**功能说明：**
设置WWDG的预分频器值，用于调整看门狗的时钟频率和超时时间。
...
**功能说明：**
设置WWDG的窗口值，定义允许喂狗的时间窗口。只有在窗口时间内喂狗才有效。
...
**功能说明：**
设置WWDG计数器的初始值，计数器值必须在0x40-0x7F范围内。
